<#
.SYNOPSIS
    Builds the Fremont Wellness watch face and installs it on Robert's Galaxy Watch 7.
...
.DESCRIPTION
    ...
    Above all it refuses to install on anything that is not a watch. The rule this replaces
    installed "if an ADB device is connected", and Robert's phone is usually connected as well.
    That is not a hypothetical: com.shieldrj.wellnesswatchface.debug was found installed on the
    phone, where a watch face does nothing at all, while the watch went without the build.
...
$PackageName = 'com.shieldrj.wellnesswatchface.debug'   # debug builds carry the .debug suffix
...
# Compare against what was just built. Observed once in the sibling repo: a deploy reported an
# older commit than HEAD because a stale APK got installed. The whole point of this script is
# that what it says is on the watch really is on the watch, so the two are checked against each
...
# --- 5. Say whether the new build is actually the face on screen -------------------------------

# Installing a watch face does not select it, so the install can succeed while Robert sees no
# change whatsoever. A watch face is the live wallpaper, so "dumpsys wallpaper" is the authority
# - the package merely being installed says nothing about what is on screen.
#
# Read only the component under "System wallpaper state:". The dump is full of decoys that also
# name a plausible watch face: mDefaultWallpaperComponent near the top, and a second
# mWallpaperComponent under "Fallback wallpaper state:" lower down. Both reported Samsung's
# DefaultWatchFace while the watch was actually showing Ultra Info Board.
$activeFace = $null
$inSystemState = $false
foreach ($line in (& adb -s $serial shell dumpsys wallpaper 2>$null)) {
    if ($line -match '^\s*System wallpaper state:') { $inSystemState = $true; continue }
    if ($line -match '^\S' -and $line -notmatch '^\s*System wallpaper state:') { $inSystemState = $false }
    if ($inSystemState -and $line -match '^\s*mWallpaperComponent=ComponentInfo\{([^}]+)\}') {
        $activeFace = $Matches[1]
        break
    }
}

Write-Host ''
if ($activeFace -and $activeFace -match [regex]::Escape($PackageName)) {
    Write-Host 'Done. This build is the face on the watch right now.' -ForegroundColor Green
} elseif ($activeFace) {
    Write-Host 'Installed, but this is NOT the face on the watch right now.' -ForegroundColor Yellow
    Write-Host "The watch is showing: $activeFace"
    Write-Host 'Nothing will look different until the face is switched: press and hold the'
    Write-Host 'watch face, swipe to "Fremont Wellness" and tap it.'
} else {
    Write-Host 'Installed. Could not tell which face is selected.' -ForegroundColor Yellow
}
